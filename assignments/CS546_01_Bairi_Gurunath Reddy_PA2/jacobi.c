...
void solve_jacobi_sequential(float matrix[MATRIX_SIZE][MATRIX_SIZE], int matrix_size, float right_hand_side[MATRIX_SIZE]);
void solve_jacobi_parallel(float matrix[MATRIX_SIZE][MATRIX_SIZE], int matrix_size, float right_hand_side[MATRIX_SIZE]);
void init_array_sequential(float array[MATRIX_SIZE], int array_size);
float clone_array_sequential(float array[MATRIX_SIZE], int array_length);
void init_array_parallel(float array[MATRIX_SIZE], int array_size);
...
int main(){
        ...
        while (user_choice == 1)
        {
                ...
                float right_hand_side[matrix_size];
...
bool check_diagoanally_dominant_parallel(float matrix[MATRIX_SIZE][MATRIX_SIZE], int matrix_size){
...
bool check_diagoanally_dominant_sequential(float matrix[MATRIX_SIZE][MATRIX_SIZE], int matrix_size){
        int check_count = 0;
        // For each row ..
        for (int i = 0; i < matrix_size; i++) {
                float row_sum = 0;
...
void solve_jacobi_sequential(float matrix[MATRIX_SIZE][MATRIX_SIZE], int matrix_size, float right_hand_side[MATRIX_SIZE]) {
       ...
       float solution[matrix_size];
        ...
        //float* last_iteration = new float[matrix_size];
           float last_iteration[matrix_size];
...
float clone_array_parallel(float array[MATRIX_SIZE], int array_length){
        //float* output = new float[array_length];
          float output[array_length];
        #pragma omp parallel for schedule (dynamic, 1)
        for (int i = 0; i < array_length; i++) {
